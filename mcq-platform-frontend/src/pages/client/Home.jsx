...
import { BookOpen, PlayCircle, ShieldCheck, ArrowRight, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[var(--color-surface)] flex items-center justify-center p-6 sm:p-8 lg:p-12 xl:p-16  relative overflow-hidden">
      ...
      <div className="relative z-10  text-center max-w-5xl lg:max-w-6xl xl:max-w-7xl mx-auto w-full space-y-8 px-4 sm:px-6 lg:px-8 pt-10">
        ...
        {/* Wider Main Icon */}
        <div className={`w-28 h-28 bg-gradient-to-br from-[var(--color-primary)]/10 to-[var(--color-secondary)]/10 border-4 border-[var(--color-primary)]/20 backdrop-blur-xl rounded-3xl flex items-center justify-center mx-auto shadow-2xl group ${isVisible ? 'animate-float-in' : ''}`}>
      ...
      <style jsx>{`
        @keyframes float-in { 0% { opacity: 0; transform: scale(0.8) translateY(20px); } 100% { opacity: 1; transform: scale(1) translateY(0); } }
        @keyframes blob { 0% { transform: translate(0px, 0px) scale(1); } 33% { transform: translate(20px, -20px) scale(1.1); } 66% { transform: translate(-20px, 20px) scale(0.9); } 100% { transform: translate(0px, 0px) scale(1); } }
        @keyframes shine { 0% { transform: translateX(-100%) skewX(-25deg); } 100% { transform: translateX(300%) skewX(-25deg); } }
