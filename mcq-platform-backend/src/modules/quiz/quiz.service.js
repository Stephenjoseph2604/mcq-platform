...
export const createQuiz = async (payload) => {
  ...
  try {
    ...
    const totalQuestions = config.reduce(
...
export const updateQuiz = async (quizId, payload) => {
  ...
  try {
    ...
    // 2️⃣ Replace rules
    await conn.execute(
      `DELETE FROM quiz_rule WHERE quiz_id = ?`,
      [quizId]
    );
    ...
    await conn.execute(
      `DELETE FROM quiz_question_config WHERE quiz_id = ?`,
      [quizId]
    );
...
export const startQuizForStudent = async (quizId, userId) => {
  ...
  try {
    // 1️⃣ Check if an attempt exists for this quiz and student
    const [attempts] = await conn.execute(
      `SELECT * FROM quiz_attempt 
       WHERE quiz_id = ? AND user_id = ? AND submitted = FALSE LIMIT 1`,
      [quizId, userId]
    );
    ...
    if (attempts.length) {
      ...
      // If questions are already locked, return them along with attemptId
    ...
    // 4️⃣ Get quiz question configuration
    ...
    // 5️⃣ Loop through each config to fetch questions
    for (const c of config) {
      let query = `SELECT id, question_text, option_a, option_b, option_c, option_d
                   FROM questions
                   WHERE category_id = ?`;
      const params = [c.category_id];

      // Technical category: fetch questions for user's department
      ...
      const [qRows] = await conn.execute(query, params);
      questionsToInsert.push(...qRows);
    }

    // 6️⃣ Lock questions in quiz_attempt_questions
    ...
    // ✅ Return both attemptId and questions
    return {
      attemptId,
  ...
  try {
    ...
    for (const ans of normalizedAnswers) {
      if (!lockedIds.includes(ans.question_id)) {
        throw new Error(`Question ${ans.question_id} was not assigned in this attempt`);
      }
    }
    ...
    for (const ans of normalizedAnswers) {
      ...
      if (!sectionTotals[question.category_id]) sectionTotals[question.category_id] = { total: 0, correct: 0 };
...
export const getStudentQuizReport = async (quizId, studentId) => {
  ...
  try {
    ...
    if (!attempt) throw new Error("No attempt found for this quiz by the student");
